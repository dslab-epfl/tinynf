...
for BENCH_KIND in throughput latency; do
  FILE="$THIS_DIR/$NF_DIR_CLEAN--$NF_KIND--$NF--$BENCH_KIND.csv"
  rm -f "$FILE"
  ...
  for LTO in $LTO_CHOICES; do
    ...
    for PERIOD in $PERIOD_CHOICES; do
      for ONEWAY in $ONEWAY_CHOICES; do
        ...
        if [ ! -f "$FILE" ]; then
          HEADER="$(printf "Period,\tLTO,\tOneway,\t%s\n" "$(head -n 1 bench.results)")"
        fi
        printf "%s,\t%s,\t%s,\t%s\n" "$PERIOD" "$LTO" "$ONEWAY" "$(tail -n +2 bench.results)" >> "$FILE"
        rm -f 'bench.results' # in case the next benchmark fails, it won't use these results by accident
      done
    done
  done
  FILE_SORTED="$(cat $FILE | sort -V)" # -V sorts the numbers within the text in a human-friendly order
  echo "$HEADER" > "$FILE"
  echo "$FILE_SORTED" >> "$FILE"
done
