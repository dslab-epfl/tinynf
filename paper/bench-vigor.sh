...
for nf in nop nat fw pol bridge lb; do
  ...
  for kind in throughput latency; do
    file="$resultsdir/$nf-$kind.results"
    rm -f "$file"
    ...
    for period in 1 2 4 8 16 32 64 128 256 512 1024; do
      ...
      for oneway in $ways; do
        ...
        TN_VIGOR_NF=vig$nf EXPIRATION_TIME=10000000 CFLAGS="-DASSUME_ONE_WAY=$oneway -DIXGBE_PIPE_SCHEDULING_PERIOD=$period -DIXGBE_PIPE_MAX_SENDS=$sends" ./bench.sh ~/tinynf/baselines/vigor $kind $layer
