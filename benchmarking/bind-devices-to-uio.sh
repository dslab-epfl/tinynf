...
for pci in $(lspci -Dmm | awk '{print $1}' | tr '\n' ' '); do
  # Get driver
  driver="$(lspci -Dvmmk | grep -F "$pci" -A 10 | grep Driver | awk '{print $2}')"
  ...
  case "$driver" in
    ...
    *)
      if echo "$@" | grep -Fq "$pci" ; then
        ...
        echo 'uio_pci_generic' | sudo tee "/sys/bus/pci/devices/$pci/driver_override" >/dev/null 2>&1
        echo -n "$pci" | sudo tee "/sys/bus/pci/drivers/uio_pci_generic/bind" >/dev/null 2>&1
      fi
      ;;
  esac
done
