...
bool tn_net_device_init(const struct tn_pci_address pci_address, struct tn_net_device* out_device)
{
	...
	*out_device = (struct tn_net_device) {
		.rx_enabled = false,
		.tx_enabled = false
	};
	...
	if (!tn_mem_phys_to_virt(dev_phys_addr, 128 * 1024, &(out_device->addr))) {
		return false;
	}
...
bool tn_net_agent_init(struct tn_net_device* input_device, size_t outputs_count, struct tn_net_device* output_devices, struct tn_net_agent* out_agent)
{
	...
	if (!tn_mem_allocate(outputs_count * sizeof(uint32_t) * TRANSMIT_HEAD_MULTIPLIER, (void**) &(out_agent->transmit_heads))) {
		TN_DEBUG("Could not allocate transmit heads for agent");
		return false;
	}

	if (!tn_mem_allocate(outputs_count * sizeof(uint64_t*), (void**) &(out_agent->rings))) {
	...
	for (size_t r = 0; r < outputs_count; r++) {
		if (!tn_mem_allocate(IXGBE_RING_SIZE * 16, (void**) &(out_agent->rings[r]))) { // 16 bytes per descriptor, i.e. 2x64bits
		...
		for (size_t n = 0; n < IXGBE_RING_SIZE; n++) {
			...
			void* packet_addr = (uint8_t*) out_agent->buffer + n * PACKET_BUFFER_SIZE;
			...
			out_agent->rings[r][n * 2u] = tn_cpu_to_le64(packet_phys_addr);
		}
		volatile uint32_t* transmit_head = &(out_agent->transmit_heads[r * TRANSMIT_HEAD_MULTIPLIER]);
		if (!tn_net_device_add_output(&(output_devices[r]), out_agent->rings[r], transmit_head, &(out_agent->transmit_tail_addrs[r]))) {
			TN_DEBUG("Could not set input");
			return false;
		}
	}

	if (!tn_net_device_add_input(input_device, out_agent->rings[0], &(out_agent->receive_tail_addr))) {
		TN_DEBUG("Could not set input");
		return false;
	}
