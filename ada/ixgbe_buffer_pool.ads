...
package Ixgbe_Buffer_Pool is
  ...
  -- NOTE: 'Size' here is one less than the desired side; cannot use 'Size-1' for the array ("discriminant in constraint must appear alone")
  type Buffer_Pool(Size: UnsignedInteger) is record
    Buffers: Buffer_Access_Array(0 .. Size);
    Index: UnsignedInteger;
  end record;

  function Buffer_Pool_Allocate(Size: in UnsignedInteger) return Buffer_Pool;
  function Buffer_Pool_Give(Pool: in out Buffer_Pool; Buf: not null access Buffer) return Boolean;
  function Buffer_Pool_Take(Pool: in out Buffer_Pool) return access Buffer;
