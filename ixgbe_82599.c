...
#define IXGBE_REG_SWSM_SMBI 0x1
#define IXGBE_REG_SWSM_SWESMBI 0x2

#define IXGBE_READ_REG(addr, reg) ???
#define IXGBE_WRITE_REG(addr, reg, value) ???

#define WAIT_WITH_TIMEOUT(result_name, timeout_in_ms, condition) bool result_name = false; for (int i = 0; i < ((timeout_in_ms) * 20); i++) { if (condition) { result_name = true; break; } usec_delay(50); }
...
static int ixgbe_lock_resources(uint64_t addr, ??? resources_mask)
{
	...
	WAIT_WITH_TIMEOUT(sw_malfunction, 10 + 3000, (IXGBE_READ_REG(addr, IXGBE_REG_SWSM) & IXGBE_SWSM_SMBI) == 0);
	...
	WAIT_WITH_TIMEOUT(fw_malfunction, 3000, (IXGBE_READ_REG(addr, IXGBE_REG_SWSM) & IXGBE_SWSM_SWESMBI) != 0);
...
static int ixgbe_unlock_resources(uint64_t addr, ??? resources_mask)
{
