...
static bool parse_hex_digit(char h, uint8_t* out_d)
{
	if (h >= 'a' && h <= 'f') {
...
static bool parse_hex8(char** ref_value, char end, uint8_t* out_result)
{
	uint8_t digit0;
	if (!parse_hex_digit(**ref_value, &digit0)) {
		return false;
	}
	...
	*ref_value = *ref_value + 1;
	...
	uint8_t digit1;
	if (!parse_hex_digit(**ref_value, &digit1)) {
		return false;
	}
...
bool tn_util_parse_pci(uint64_t count, char** values, struct tn_pci_device* out_devices)
{
	for (uint64_t n = 0; n < count; n++) {
		uint8_t bus;
		if (!parse_hex8(&(values[n]), ':', &bus)) {
			return false;
		}

		uint8_t device;
		if (!parse_hex8(&(values[n]), '.', &device)) {
			return false;
		}

		uint8_t function;
		if (!parse_hex8(&(values[n]), '\0', &function)) {
			return false;
		}
